...
def model(input_shape, eps = 1e-4):
    ...
    conv_net = MaxPool1D(pool_size=5)(conv_net)
    ...
    dense_net = Dense(512, kernel_regularizer=l2(eps),
    ...
    emb_input = Input(shape=(512,), name='emb_input')
    deconv_net = Dense(256, kernel_regularizer=l2(eps),
                    bias_regularizer=l2(eps))(emb_input)
    deconv_net = LeakyReLU(alpha=0.1)(deconv_net)
    deconv_net = Reshape((1, 256))(deconv_net)
    ...
    deconv_net = UpSampling1D(size=5)(deconv_net)
